...
public class Proj6BaseV2
{
	...
	public static void main (String[] args)
	{
		...
		graph airports = new graph();  // the aiports
		FilesInUse<String> files = new FilesInUse<String>(); //files in use
		...
		try {  //try-catch clauses are needed since BufferedReader and Scanner classes
			...
			while (inline != null)
			{
				...
				if (command.equals("q") == true)
					...
					doTravel(sc, airports, files);

				else if (command.equals("r") == true)
					doResize(sc, airports, files);

				else if (command.equals("i") == true)
					doInsert(sc, airports, files);

				else if (command.equals("d") == true)
					doDelete(sc, airports, files);

				else if (command.equals("l") == true)
					doList(sc, airports, files);
	...
	public void doTravel(Scanner sc, graph airports, FilesInUse<String> files)
	{
		int val1 = 0;
		int val2 = 0;
		...
		System.out.println ("Performing the Travel Command from " + val1 +
				" to " + val2);
		if(val1 > airports.n || val1 <= 0 || val2 > airports.n || val2 <= 0){ //in range
	...
	public void doResize(Scanner sc, graph airports, FilesInUse<String> files)
	{
		int val1 = 0;
        ...
        if(val1 <= 0){ System.out.println ("Need at least 1 airport");} //checking to see if within range
        else{
		airports.resize(val1);
        }
	}

	public void doInsert(Scanner sc, graph airports, FilesInUse<String> files)
	{
		int val1 = 0;
		int val2 = 0;
	...
	public void doDelete(Scanner sc, graph airports, FilesInUse<String> files)
	{
		int val1 = 0;
		int val2 = 0;
		...
		System.out.println ("Performing the delete command from airports " + val1 +
				" to " + val2);
		if(val1 > airports.n || val1 <= 0 || val2 > airports.n || val2 <= 0){ //range check
			System.out.println ("\nNumbers not in range try again.");
		}
		else{
	...
	public void doList(Scanner sc, graph airports, FilesInUse<String> files)
	{
		airports.show();
	...
	public void doFile(Scanner sc, graph airports, FilesInUse<String> files)
	{
		...
		try{
			...
			if(files.contains(fname)){  //check if in use
				System.out.println ("\n\nFile already in use\n\n");
				return;
			}
